...
#' Title
#'
#' @param ionMode Positive or negative 1 is pos, -1 is neg
#' @param ppm PPM value
#' @param database Name of database (kegg, pubchem, hmdb)
...
#' Importing peak file
#'
#' @param PeakFile Name of input peak file
#'
#' @return
#' @export
#'
ImportMSPeaks <- function(PeakFile = NA) {
  ...
  dataSet$exp.mat[["peakRaw"]] <- data.frame(rep(0, nrow(raw_data)));
  rownames(dataSet$exp.mat[["peakRaw"]]) <- raw_data$id;
...
#' Perform global network optiomization
#'
#' @return
#'
