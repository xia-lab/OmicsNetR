...
)
{
  ...
  ion_mode = Mset$ParamSet$polarity;
  raw = Mset$raw_data %>%
  ...
  {
    s4=s3 %>%
      mutate(mean_inten = Sample) %>%
      filter(mean_inten > inten_cutoff)
    # NOTE: Zhiqiang Pang Modified this part
  }
...
expand_library = function(lib, rule, direction, category){
  ...
  formula_exp = fast_calculate_formula(formula_lib, formula_rule, sign = as.integer(direction))
  #formula_exp = my_calculate_formula(formula_lib, formula_rule, sign = as.integer(direction))
...
# Expand_libraryset ####
Expand_libraryset = function(LibrarySet, Mset = NULL){
  ## initialize
...
Multicharge_isotope_connection = function(EdgeSet, EdgeSet_oligomer_multicharge){
  ...
  edge_multicharge_isotope = EdgeSet_natural_abundance %>%
    ...
    #Note: must use my_calculate_formula rather than the fast Rcpp bc the num type is not compatible
...
Heterodimer_connection = function(peak_group, NodeSet, ppm_tol = 10, inten_threshold = 1e5){
  ...
  ## Fast core with Rcpp to speed up -Zhiqiang Pang
...
Library_MS2_fragment_connection = function(peak_group, 
                                           StructureSet, 
                                           MS2_library,
                                           inten_threshold = 1e5,
                                           ppm_tol = 10, 
...
propagate_oligomer = function(new_nodes_df, sf, EdgeSet_oligomer, NodeSet, current_step){
  ...
  new_nodes_df1 = new_nodes_df %>% 
    ...
    mutate(node_id = node2, 
           mass = mass * linktype,
           formula = as.character(mapply(fast_calculate_formula, formula, formula, sign = linktype - 1)),
           rdbe = rdbe * linktype) %>%
...
propagate_multicharge = function(new_nodes_df, sf, EdgeSet_oligomer, NodeSet, current_step){
  ...
  new_nodes_df2 = new_nodes_df %>% 
    ...
    mutate(node_id = node1, 
           mass = mass / linktype,
           formula = as.character(mapply(fast_calculate_formula, formula, formula, sign = -(linktype-1)/linktype)),
           rdbe = rdbe / linktype) %>%
...
propagate_heterodimer = function(new_nodes_df, sf, EdgeSet_heterodimer, NodeSet, 
  ...
  heterodimer_ls <- propagate_heterodimer_core(new_nodes_df_heterodimer, sf, 
                                               propagation_category = temp_propagation_category, 
                                               node_mass = node_mass, ppm_threshold = propagation_ppm_threshold);
  heterodimer_ls <- 
    lapply(heterodimer_ls, function(x){
                                  ...
                                  record_ppm_tol = 5e-6)
{
  ...
  EdgeSet_all <- Mset$EdgeSet_all;
  NodeSet <- Mset$NodeSet;
  sf <- StructureSet <- Mset$StructureSet;

  empirical_rules = Mset$empirical_rules
  propagation_rule = Mset$propagation_rule
  node_mass = sapply(NodeSet, "[[", "mz") %>% sort()
  node_RT = sapply(NodeSet, "[[", "RT")[names(node_mass)]
  temp_id = as.numeric(names(node_mass)) # numeric
...
Score_structureset_RT = function(StructureSet_df, NodeSet, LibrarySet, 
  ...
  node_RT = sapply(NodeSet, "[[", "RT")
  library_RT = LibrarySet$rt;
  if(is.null(library_RT)) {
    library_RT <- rep(NA, length(LibrarySet$library_id))
  }
  names(library_RT) = LibrarySet$library_id
...
Score_structureset_MS2 = function(StructureSet_df, 
                                  NodeSet, 
                                  Mset,
                                  # only spectra matching/similarity score > cutoff, then nodes get bonus
                                  MS2_match = 1, 
                                  MS2_match_cutoff = 0.5, 
                                  MS2_similarity = 0.5, 
...
Score_structureset = function(PeakSet){
  ...
  structureset_database_origin <-
    Score_structureset_database_origin(StructureSet_df, 
                                       PeakSet$LibrarySet,
                                       database_match = 0.5, 
                                       manual_match = 1)
  
  structureset_mz <- Score_structureset_mz(StructureSet_df, 
                                           PeakSet$NodeSet, 
                                           score_ppm_error_rate = -.5)
  
  structureset_RT <- Score_structureset_RT(StructureSet_df, 
                                          PeakSet$NodeSet, 
                                          PeakSet$LibrarySet, 
                                          rt_match = 1, 
                                          known_rt_tol = 0.5)
  
  structureset_MS2 <- Score_structureset_MS2(StructureSet_df, 
                                            PeakSet$NodeSet, 
                                            Mset = PeakSet,
                                            # only spectra matching/similarity score > cutoff, then nodes get bonus
                                            MS2_match = 1, 
  ...
  structureset_missing_isotope <- Score_structureset_missing_isotope(StructureSet_df, 
                                                                    PeakSet$NodeSet, 
                                                                    PeakSet$EdgeSet_all,
                                                                    isotope = c("Cl"), 
                                                                    isotope_penalty = c(-1), 
...
# initiate_ilp_nodes ####
initiate_ilp_nodes = function(Mset){
  StructureSet_df <- Mset$StructureSet_df;
  # summing all scores
  {
    score_sum = StructureSet_df %>% 
      dplyr::select(starts_with("score")) %>% 
...
initiate_ilp_edges = function(EdgeSet_all, CplexSet, Exclude = ""){
  ...
  node_id_ilp_node_mapping = split(ilp_nodes_assigned$ilp_node_id, 
                                   ilp_nodes_assigned$node_id)
...
Score_inten_isotope = function(ilp_edges, NodeSet, 
  ...
  ilp_edges_isotope = ilp_edges %>%
    ...
    mutate(inten_ratio_measured = inten2 - inten1, 
           inten_ratio_calculated = log10(mapply(isotopic_abundanceU, .$formula1, .$linktype)),
...
isotopic_abundanceU <- function(formula = "C2H4O1",
                                isotope = "[13]C1"){
  isotopic_abundance(formula, isotope)[1]
}
...
initiate_heterodimer_ilp_edges = function(EdgeSet_all, CplexSet, NodeSet){
  ...
  for(i in 1:nrow(EdgeSet_df)){
    ...
    formula_transform_all = fast_calculate_formula(formula1, transform,1)
...
initiate_g_met = function(CplexSet){
  ...
  g_met = igraph::graph_from_data_frame(ilp_edges_met, 
                                directed = T, 
...
initiate_g_nonmet = function(CplexSet, solution){
  ...
  g_nonmet = igraph::graph_from_data_frame(ilp_edges_weights, 
                                   directed = T, 
...
initiate_met_dist_mat = function(g_met, CplexSet, core_annotation){
  ...
  distMatrix <- igraph::shortest.paths(g_met, 
                               v=core_met_names,
                               to=V(g_met),
                               mode = "all") 
...
initiate_nonmet_dist_mat = function(g_nonmet, CplexSet, core_annotation, 
  ...
  distMatrix <- igraph::shortest.paths(g_nonmet, 
                               v=core_names,
                               to=target_names, 
...
track_annotation_met = function(query_ilp_id, 
  ...
  {
    cat("query_ilp_id is ", query_ilp_id, "\n")
    shortest_ilp_nodes = which(query_distMatrix == query_distMatrix_min)
    parent_selected = names(shortest_ilp_nodes)[1]
    paths_connect_ij_nodes = shortest_paths(g_annotation, 
                                            from = parent_selected, 
...
path_annotation = function(CplexSet, NetworkSet, solution = "ilp_solution"){
  ...
  #   filter(T)
   ...
   CplexSet$ilp_nodes = ilp_nodes %>%
     mutate(path = path_annotation) %>%
     filter(T)
   CplexSet$core_annoted_met <- core_annotation_unique_met %>% 
...
all_network = function(g_met, g_nonmet){
  ...
  g_met2 = igraph::graph_from_data_frame(g_met2_edges,
  ...
  g_nonmet2 = igraph::graph_from_data_frame(g_nonmet2_edges,
  ...
  g_all = igraph::graph_from_data_frame(g_all_edges,
                                directed = F,
...
valid_network = function(g_all){
  ...
  # Solutions - Only retain subnetwork where step=0 annotation exist
  {
    ...
    g_all_valid = igraph::graph_from_data_frame(g_all_edges_valid,
                                        directed = F,
