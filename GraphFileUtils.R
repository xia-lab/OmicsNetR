...
ReadGraphFile <- function(fileName, fileType) {
  ...
  if(fileType == "graphml"){
    ...
    dataList <- readRDS("../../data/test/metGeneNet.rds");
    data.org <<- "hsa";
    graphX <- readRDS("../../data/test/igraphObj.rds")
    library(NetPathMiner);
    v.layout <- layoutVertexByAttr(graphX, "pathway")
    V(graphX)$x <- -v.layout[,1]
    V(graphX)$y <- -v.layout[,2]
    newlist <- list(nodes=dataList$nodes1, edges=dataList$edges1)
    netPropU <<- newlist
    library(Cairo);
    Cairo(file = "netCluster.png", width=800, height=600, type="png", bg="white");
    plotNetwork(graphX, vertex.color="pathway", layout=v.layout)
    dev.off();
  }else{
